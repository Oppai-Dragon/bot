...
getKeysTelegramTest =
    TestCase $
    assertEqual "for (getKeysTelegram testUpdatesObj)"
    (HM.singleton "chat_id" (Number 1))
    $ fst $ unsafePerformIO $
    runStateT (runReaderT (getKeysTelegram testUpdatesObj) Telegram) testConfig

updateTelegramTest =
    TestCase $
    assertEqual "for (updateTelegram )"
    "suka"
    $ fst $ unsafePerformIO $
...
getTelegramMsgTest =
    TestCase $
    assertEqual "for (getTelegramMsg )"
    "suka"
    $ fst $ unsafePerformIO $
