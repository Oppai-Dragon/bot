...
getKeysVkTest =
    TestCase $
    assertEqual "for (getKeysVk testUpdatesObj)"
    testUpdatesObj
    $ fst $ unsafePerformIO $
...
getAttachmentTest =
    TestCase $
    assertEqual "for (getAttachment [attachmentsObj])"
    ",audio174435367_456241022"
    $ getAttachment [attachmentsObj]

getVkMsgTest =
    TestCase $
    assertEqual "for (getVkMsg)"
    ""
    $ fst $ unsafePerformIO $
    runStateT (runReaderT getVkMsg testUpdatesObj) testConfig


testResponseObj = HM.fromList
    [("ts",Number 1)
    ,("updates",Object testUpdatesObj)
    ]

testUpdatesObj = HM.fromList
    [("object",Object (HM.fromList
        [("message",Object (HM.fromList
            [("attachments",(Array . V.fromList) [Object attachmentsObj]
             )
            ,("text",String "")
...
attachmentsObj = HM.fromList
    [("type",String "audio")
    ,("audio",Object (HM.fromList
        [("url",String "https://cs9-21v4.vkuseraudio.net/p5/899903aa90615c.mp3?extra=wY1ebEmN2-sMSC_w4FX3IEIQVUdj7fG0hDR83Ae8wv1C45z8RB5rIV2PEuYRNf_Ek0E6iIg6pCjmfuek27On69bZJLeJa6u_2jEFtM9V4ne-v8bQcK9pISBWkgnM9Kh4UyLDAq21H59n9yKqiiUGRdY&long_chunk=1")
        ,("main_artists",(Array . V.fromList) [Object (HM.fromList
